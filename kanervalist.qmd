...
```
Of course, while this approach is easy, it is not practible. 
...
once again, how given an arbitrary $l$ how would we recover the head?
...
[from @kanervaComputing10000bitWords2014], and fairs better against @prp-minimal-encoding-debt.
We will include the previous $\mathbf{nil}, \mathbf a, \mathbf b, \dots$ in
our codebook. Further, we will use two additional operations: the *left permutation*
$\Pi_\leftarrow$ and the *right permutation* $\Pi_\rightarrow$, where:
$$
\begin{align*}
...
def Memory_recall(x):
    global Memory
    activations = [HRR.similarity(mem, x) for x in Memory]
    return HRR.from_array(Memory[np.argmax(activations), :])


def Memory_store(x):
    global Memory
    x = np.expand_dims(x, axis=-1)
    Memory = np.concatenate((Memory, x), axis=-1)
